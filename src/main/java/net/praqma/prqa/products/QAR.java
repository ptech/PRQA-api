...
public class QAR implements Product {
	...
	/**
	 * QAR is invoked using QAW where this is taken as parameter in the QAW
	 * command.
	 */
	...
	public final String getProductVersion(Map<String, String> environment, File workspace, boolean isUnix) throws PrqaSetupException {
		...
		try {
			...
			throw new PrqaSetupException(String.format("Failed to detect QAR running this command %s, exit code was %s%nMessage was:%n%s", ex.getCommand(),
					ex.getExitValue(), ex.getMessage()), ex);
		}
